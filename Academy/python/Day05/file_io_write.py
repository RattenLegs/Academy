...
# 예외처리를 통해 안전하게 작성합니다.
try:
    # 파일 저장기능 (write, append)
    file_path = r'C:\Users\user\Desktop\LKM\python\test\test2.txt' # 생성'할' 파일명까지 작성
    ...
    # text = 'w모드를 이용하여 이미 존재하는 파일 경로를 지정하면 덮어씁니다.'
    text = 'a모드는 기존 파일만 지정할 수 있는 것이 아닌 새 파일도 지정 가능합니다.'

    # 파일을 저장할 때 f.write() 메서드를 사용합니다.
    # 저장할 데이터를 인수로 전달합니다.
    f.write(text)
    print('파일 저장 완료!')
except:
