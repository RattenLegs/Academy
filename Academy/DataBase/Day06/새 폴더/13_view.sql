...
기본 테이블로 유도된 가상 테이블이기 때문에 필요한 컬럼만 저장해두면 관리가 용이
view는 가상테이블로 실제 데이터가 물리적으로 저장된 형태는 아님
view를 통해 데이터에 접근하면 원본 데이터는 안전하게 보호될 수 있음
*/

SELECT * FROM user_sys_privs;

-- 단순 뷰
-- 뷰의 컬럼 이름은 함수같은 가상표현식이면 안되고 꼭 별칭을 지어야함
...
-- 뷰 수정(CREATE OR REPLACE VIWE 구문)
...
/*
VIEW에 INSERT가 일어나는 경우 실제 테이블에도 반영됩니다
그래서 VIEW의 INSERT, UPDATE, DELETE는 많은 제약사항이 따른다

- 원본 테이블의 특정 컬럼이 NOT NULL인 경우 VIEW에 INSERT 불가
...
-- 원본 테이블의 null을 허용하지 않는 컬럼때문에 들어갈 수 없습니다
...
-- 읽기 전용 뷰 -> WITH READ ONLY(DML 연산을 막음
CREATE VIEW view_emp_test AS (
    SELECT
        employee_id,
        first_name,
