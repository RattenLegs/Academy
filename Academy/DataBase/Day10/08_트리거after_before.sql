...
CREATE OR REPLACE TRIGGER trg_user_backup
    ...
    ON tbl_user
    FOR EACH ROW -- 특정행: 뒤에 WHERE 조건절
DECLARE -- 사용할 변수를 선언하는 곳
    v_type VARCHAR2(10);
BEGIN
    IF UPDATING THEN -- UPDATEING은 시스템 자체에서 상태에 대한 내용을 지원하는 빌트인 구문(일종의 키워드)
        v_type := '수정';                     -- UPDATING, INSERTING, DELETING 등이 있다
    ELSIF DELETING THEN
        v_type := '삭제';
    END IF;
    
    -- 실행 구문 시작 (:OLD는 테이블 DELETE, UPDATE가 적용되기 전 기존 데이터. 즉 변경 전 데이터), Main에는 변경된 데이터 백업에는 변경전 데이터
...
CREATE OR REPLACE TRIGGER trg_user_insert
    ...
    FOR EACH ROW
BEGIN
    :NEW.name := SUBSTR(:NEW.name, 1, 1) || '**'; -- 앞의 한글자만 취득하고, 나머지는 **로 붙이겠다
END;
...
BEGIN
    ...
    -- v_product_no := :NEW.product_no; 지우고 WHERE product_no = :NEW.product_no; 로 바꿔도됨
END;
