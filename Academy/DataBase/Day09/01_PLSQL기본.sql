...
DECLARE -- 변수 선언하는 구간 (선언부)
    emp_num NUMBER; -- 변수 선언(타입+변수명)
...
특이한 점은 SELECT절 아래에 INTO절을 사용해 조회결과를 변수에 할당할 수 있습니다.
*/

DECLARE
    v_emp_name VARCHAR2(50); -- 사원명 변수 (문자열 변수는 길이제약 필수)
...
BEGIN
    SELECT
        ...
        d.department_name
    INTO
        v_emp_name, v_dep_name -- 변수에 조회한 결과 대입을 의미
    FROM employees e
...
-- 테이블명.컬럼명%TYPE을 사용함으로써 타입을 일일히 확인하는 번거로움을 방지할 수 있따.
DECLARE
    v_emp_name employees.first_name%TYPE; -- 찾아서 정리(복습 중에 있음)
