...
-- 찹아서 SELECT 이하에 있는 컬럼들을 출력하는 쿼리문
...
--e.department_id와 d.department_id가 다를 수 있음(ex: 전자는 실제 일함, 후자는 만들어놓은것만)
...
AND loc.state_province = 'California'; --1(일반조건, 일반조건>조인)
...
어느 한 테이블에 공통값이 없더라도 해당 row들이 조회 결과에
모두 포함되는 조인을 말합니다.
...
/*
외부조인 사용시 조건을 걸 때, 조건에도 (+)를 붙여야 한다.
외부조인 진행 시 모든 조건에 (+)를 붙여야 하며
일반조건에도 (+)를 붙이지 않으면 데이터가 누락되는 현상이 발생

외부조건은 NULL을 포함하더라도 다 가져오는게 목적이기 때문에 일반조건 자체에 큰 힘이 발휘하지 않음
*/